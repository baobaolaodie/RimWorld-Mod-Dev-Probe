...
-   **详细探查**：展示目标类型的所有字段（Fields）、属性（Properties）和方法（Methods），包括其参数类型和返回值。支持探测**非公开 (NonPublic)** 成员（私有方法、字段等），便于 Harmony 补丁开发。
...
### v1.0.1 (2026-02-14)
*   **功能增强**:
    *   **非公开成员探测**: 现在支持探测 `NonPublic` (私有/内部) 的方法、字段和属性。
    *   **字段可见性标识**: 字段列表中会标注 `(public)` 或 `(non-public)`。
    *   **用途**: 此更新主要用于辅助 Harmony 补丁开发，因为 Patch 经常需要针对私有方法或字段进行操作。
